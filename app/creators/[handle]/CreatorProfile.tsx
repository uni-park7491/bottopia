...
export default function CreatorProfile({ handle }: { handle: string }) {
  ...
  return <main className="creator-profile-shell">
    ...
    <section className="creator-profile-hero">
      <div className="creator-avatar-badged"><CreatorAvatar className="creator-profile-avatar" handle={profile.handle} name={profile.displayName} /><CreatorBadge role={profile.role} /></div>
      <div className="creator-profile-title"><h1>{profile.displayName}</h1><span>@{profile.handle}</span></div>
      <div className="creator-profile-bio"><p>{profile.bio || '작품과 제작 과정을 공개하는 BOTTOPIA 크리에이터입니다.'}</p>{profile.availableForWork && <div className="creator-profile-availability"><span className="creator-availability">의뢰 가능</span></div>}<dl><div><dt>BASE</dt><dd>{profile.location || '—'}</dd></div><div><dt>TOOLS</dt><dd>{profile.tools || '—'}</dd></div><div><dt>WORKS</dt><dd>{works.length}</dd></div></dl>{links.length > 0 && <nav className="creator-social-links" aria-label="Creator social links">{links.map(([label, url]) => <a key={label} href={url} target="_blank" rel="noopener noreferrer">{label} ↗</a>)}</nav>}</div>
