...
export default function ProfileEditor({ email, siteHost }: { email: string; siteHost: string }) {
  ...
  useEffect(() => {
    ...
    fetch('/api/profile').then(async (response) => {
      ...
      if (active) { setProfile(data.profile); setPersisted(Boolean(data.persisted)); setStatus('idle'); }
  ...
  return <main className="profile-editor-shell">
    <header className="creator-page-header"><Link className="brand" href="/">BOT<span>•</span>TOPIA</Link><nav><Link href="/creators">CREATORS</Link><Link href="/studio">CREATOR STUDIO</Link><MemberLogin compact /></nav></header>
    <section className="profile-editor-intro"><p className="eyebrow">CREATOR IDENTITY / PUBLIC PROFILE</p><h1>SHOW THE MAKER<br />BEHIND THE WORLD.</h1><p>표시 이름, 소개, 주로 사용하는 도구와 외부 채널을 연결하세요. 이메일은 공개되지 않습니다.</p></section>
    {status === 'loading' ? <div className="creator-loading" role="status">LOADING CREATOR PROFILE…</div> : !profile ? <div className="creator-loading" role="alert">{message || '프로필을 불러오지 못했습니다.'}</div> : <section className="profile-editor-grid">
      <aside className="creator-profile-card profile-live-preview">
        <div className="creator-avatar" aria-hidden="true">{creatorInitials(profile.displayName)}</div>
        ...
        {persisted && <Link href={`/creators/${profile.handle}`}>VIEW PUBLIC PROFILE ↗</Link>}
      </aside>
